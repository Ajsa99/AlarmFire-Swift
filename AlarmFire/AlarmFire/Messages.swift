...
struct NotificationData: Decodable, Identifiable {
    let id: Int
    let fireDepartment: Bool
    let hospital: Bool
    let policeStation: Bool
...
struct Messages: View {
                                    ...
                                    HStack {
                                        if let locationName = locationNames[notification.id] {
                                            ...
                                            HStack(spacing: 10) {
                                                ...
                                                if notification.policeStation {
                                                    Image(systemName: "bell.fill")
                                                        .foregroundColor(.blue)
                                                }
                                                if notification.hospital {
                                                    Image(systemName: "bell.fill")
                                                        .foregroundColor(.red)
                                                }
                                                if notification.fireDepartment {
                                                    Image(systemName: "bell.fill")
                                                        .foregroundColor(.yellow)
                                                }
                                                // Ako nijedna nije označena, prikaži sivo zvonce
                                                if !notification.fireDepartment && !notification.policeStation && !notification.hospital {
                                                    Image(systemName: "bell.fill")
    ...
    private func fetchNotifications() {
        ...
        guard let url = URL(string: "http://alarmfire-001-site1.dtempurl.com/Notification/GetNotifications") else {
